import nielsenimoveis, { adapter } from '../src/sites/nielsenimoveis';

describe('Nielsen Imoveis Site', () => {
    it('should have correct configuration', () => {
        expect(nielsenimoveis.name).toBe('nielsenimoveis.com.br');
        expect(nielsenimoveis.driver).toBe('axios');
        expect(nielsenimoveis.url).toContain('nielsenimoveis.com.br');
        expect(nielsenimoveis.getPaginateParams(2)).toEqual({ params: { pagina: 2 } });
    });
    ...
    it('should parse HTML with one property successfully', async () => {
        ...
        expect(result.imoveis[0].link).toBe('https://www.nielsenimoveis.com.br/imovel/123/casa');
    });
