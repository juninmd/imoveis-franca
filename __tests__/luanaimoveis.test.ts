import site, { adapter } from '../src/sites/luanaimoveis';
...
describe('luanaimoveis site', () => {
    it('should match fastimob adapter signature and exported object', async () => {
        ...
        expect(site.name).toBe('luanaimoveis.com.br');
        expect(site.driver).toBe('axios');
        expect(site.url).toBe('https://www.luanaimoveis.com.br/imoveis');
        expect(typeof site.getPaginateParams).toBe('function');

        const pageParams = site.getPaginateParams(2);
        expect(pageParams.path || pageParams.payload?.url || (pageParams as any).url).toBe('https://www.luanaimoveis.com.br/imoveis?page=2');
    });
