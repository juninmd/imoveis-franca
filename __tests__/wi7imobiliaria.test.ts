import site, { adapter } from '../src/sites/wi7imobiliaria';

describe('wi7imobiliaria site', () => {
    it('should match fastimob adapter signature and exported object', async () => {
        expect(site.enabled).toBe(true);
        expect(site.name).toBe('wi7imobiliaria.com.br');
        expect(site.driver).toBe('axios');
        expect(site.url).toBe('https://www.wi7imobiliaria.com.br/imoveis');
        expect(typeof site.getPaginateParams).toBe('function');

        const pageParams = site.getPaginateParams(2);
        expect(pageParams.path || pageParams.payload?.url || (pageParams as any).url).toBe('https://www.wi7imobiliaria.com.br/imoveis?page=2');
    });
    ...
    it('should parse mock HTML correctly', async () => {
        const mockHtml = `
            ...
            <div class="recent-properties-box">
                <a href="/imoveis/detalhes/456"><img class="img-responsive" src="/img/2.jpg" /></a>
                <div class="tag-f">locacao</div>
                <h3 class="title"><a href="/imoveis/detalhes/456">Casa Aluguel</a></h3>
                <div class="location">Jardim Tropical, Franca</div>
