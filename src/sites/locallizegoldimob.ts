...
export default {
  enabled: true,
  tipo: 'venda',
  url: 'https://locallizegoldimob.com.br/comprar/todos',
  name: 'locallizegoldimob.com.br',
  driver: 'axios',
  itemsPerPage: 15,
  params: [],
  getPaginateParams: (page: number) => {
    return { url: `https://locallizegoldimob.com.br/comprar/todos?page=${page}` };
  },
  adapter,
} as Site;
...
export async function adapter(html: string): Promise<{ imoveis: Imoveis[], qtd: number, html: string }> {
  ...
  const imoveis: Imoveis[] = sourceImoveis.map((imv: any) => {
    ...
    if (cat && city && dist && code) {
      link = `https://locallizegoldimob.com.br/imovel/venda/${cat}/${city}/${dist}/${code}`;
    }
