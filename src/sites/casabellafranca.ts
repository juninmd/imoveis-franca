...
export default {
  enabled: true,
  tipo: 'venda',
  url: 'https://www.casabellafranca.com.br/imoveis?pretensao=venda',
  name: 'casabellafranca.com.br',
  driver: 'axios',
  itemsPerPage: 12,
  params: [],
  getPaginateParams: (page: number) => {
    return { url: `https://www.casabellafranca.com.br/imoveis?pretensao=venda&page=${page}` };
  },
  adapter,
} as Site;
...
export async function adapter(html: string): Promise<{ imoveis: Imoveis[], qtd: number, html: string }> {
  ...
  $('.thumbnail.recent-properties-box').each((_, el) => {
     const titleNode = $(el).find('.detail h1 a').length ? $(el).find('.detail h1 a') : $(el).find('.detail h3 a');
     const locationNode = $(el).find('.location').text().trim();
     ...
     $(el).find('.facilities-list li').each((_, fac) => {
         ...
         if (t.includes('garagem')) garage = num;
     });
